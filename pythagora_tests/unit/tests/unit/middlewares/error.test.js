let ApiError = require('../../../../../src/utils/ApiError');
...
describe('Extended Error middlewares Tests', () => {
  describe('Error converter Extended Tests', () => {
    test('should validate if the Error response has property name', () => {
      const error = new Error('Any error');
      error.statusCode = httpStatus.BAD_REQUEST;
      const next = jest.fn();
      errorConverter(error, httpMocks.createRequest(), httpMocks.createResponse(), next);
      expect(next).toHaveBeenCalled();
      expect(next.mock.calls[0][0]).toHaveProperty('name');
    });

    test('should validate if the Error response has property isOperational', () => {
      const error = new ApiError(httpStatus.BAD_REQUEST, 'Any error');
      const next = jest.fn();
      errorConverter(error, httpMocks.createRequest(), httpMocks.createResponse(), next);
      expect(next).toHaveBeenCalled();
      expect(next.mock.calls[0][0]).toHaveProperty('isOperational');
    });
  });
  
  describe('Error handler Extended Tests', () => {
    test('should return BAD_REQUEST status code when error is operational', () => {
      config.env = 'production';
      const error = new ApiError(httpStatus.BAD_REQUEST, 'Any error');
      const res = httpMocks.createResponse();
      errorHandler(error, httpMocks.createRequest(), res);
    ...
    test('should return INTERNAL_SERVER_ERROR status code when error is not operational', () => {
      config.env = 'production';
      const error = new ApiError(httpStatus.BAD_REQUEST, 'Any error', false);
      const res = httpMocks.createResponse();
      errorHandler(error, httpMocks.createRequest(), res);
